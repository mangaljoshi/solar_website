...
<div class="thumbtack-container"> 
    <div class="thumbtack-heading">
        <h1>Additional home services from top pros through our trusted partner - Thumbtack</h1>
    ...
    <div class="serch-box">
       <span>
        ...
        </span><span><input id="zipCode" class="zipcode input-2" placeholder="Enter your zip code" type="tel" name="zipCode"  value="{{ isset($zipCode) ? $zipCode : (isset(('userData')['zip_code']) ? ('userData')['zip_code'] : '') }}" maxlength="5">
        </span>
        <span><button id="tt-btn" type="submit" class="thumb-tack-btn"><i class="fa-solid fa-magnifying-glass"></i></button></span> 
    ...
    <p id="thumbtack-error" style="color: red; font-size: 18px; font-weight: bold; display:none"> Please Enter valid Zip Code</p>
...
<div class="container-11">
    ...
    <a class="subscribe-button" href="{{ $item['thumbtack_url'] }}" target="_blank">View profile</a>
            ...
            <div class="reviews">
                ...
                <span class="rating">
                    ...
                    @for ($i = 0; $i < $fullStars; $i++)
                        <i class="fa-solid fa-star" style="color: #ffc800; margin-right: 0;"></i>
                    @endfor
                    ...
                    @for ($i = 0; $i < $emptyStars; $i++)
                        <i class="fa-regular fa-star" style="margin-right: 0;" ></i>
                    @endfor
                   <span style="opacity: .8">({{ $item['num_reviews'] }})</span> 
                </span>
            </div>
            <span><i class="fa-solid fa-location-dot"></i> Servicing {{ $item['location'] }}</span><br>
            <span><i class="fa-solid fa-user-large"></i> {{ $item['years_in_business'] }} years in business</span><br>
            <span><i class="fa-solid fa-medal"></i> Hired {{ $item['num_hires'] }} times</span><br>
        </div>

        @if(!empty($item['featured_review']))
        <p class="description">
            {{ substr($item['featured_review'], 0, 200) }} <span id="see-more-{{ $loop->index }}" style="display:none;">{{ substr($item['featured_review'], 100) }}</span>
        ...
        <a class="subscribe-button-1" style="display: none" href="{{ $item['thumbtack_url'] }}" target="_blank">View profile</a>
